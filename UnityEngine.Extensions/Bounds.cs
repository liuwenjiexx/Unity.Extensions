...
namespace UnityEngine.Extensions
{
    public static partial class UnityExtensions
    {
        ...
        public static bool IntersectRay(this Bounds bounds, Ray ray, Transform t )
        {
            Matrix4x4 mat;
            mat = t.worldToLocalMatrix;
            Ray ray2 = new Ray(mat.MultiplyPoint(ray.origin), mat.MultiplyVector(ray.direction));

            return bounds.IntersectRay(ray2);
        }
        ...
        public static IEnumerable<Vector3> IntersectPlane(this Bounds bounds, Plane plane)
        {
            Vector3 point1 = Vector3.zero, point2 = Vector3.zero;
            bool first = true;
            foreach (var point in EnumerateLines(bounds))
            {
                if (first)
                {
                    point1 = point;
                ...
                if (!first)
                {
                    ...
                    Vector3 dir;
                    dir = point2 - point1;
                    Ray ray = new Ray(point1, dir);
                    if (plane.Raycast(ray, out dist) && dist <= dir.magnitude)
                    {
                        yield return ray.GetPoint(dist);
                    }
                }
                first = !first;
            }
